...
const mainServices = [
  {
    ...
    features: ['Living room design', 'Bedroom suites', 'Kitchen renovation', 'Bathroom design', 'Home office setup'],
    price: 'From $2,500',
  },
  {
    icon: Building,
    ...
    features: ['Office layouts', 'Retail design', 'Restaurant interiors', 'Hotel lobbies', 'Co-working spaces'],
    price: 'From $5,000',
  },
  {
    icon: Layout,
    ...
    features: ['Floor plan design', 'Furniture placement', 'Traffic flow optimization', '3D visualization', 'CAD drawings'],
    price: 'From $800',
  },
];
...
const additionalServices = [
  ...
  },
  {
    ...
    description: 'Professional color selection and coordination for harmonious interior schemes.',
  },
  {
    icon: Ruler,
  ...
  },
  {
    ...
    description: 'Sourcing and purchasing of furniture, fixtures, and decorative elements.',
  },
];
...
const process = [
  ...
  },
  {
    ...
    description: 'We help you choose the perfect materials, colors, and finishes for your space.',
  },
  {
    step: '04',
...
export default function Services() {
  ...
  const { ref: servicesRef, hasIntersected: servicesIntersected } = useIntersectionObserver();
  const { ref: additionalRef, hasIntersected: additionalIntersected } = useIntersectionObserver();
  const { ref: processRef, hasIntersected: processIntersected } = useIntersectionObserver();
  ...
  return (
    <div className="min-h-screen bg-warm-white">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={servicesRef} className={`text-center mb-16 reveal ${servicesIntersected ? 'active' : ''}`}>
            <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gradient">Core Services</h2>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {mainServices.map((service, index) => {
              ...
              return (
                <div
                  key={service.title}
                  className={`service-card bg-white rounded-2xl p-8 shadow-lg border border-gray-100 reveal ${servicesIntersected ? 'active' : ''}`}
                ...
                >
                  ...
                  <h3 className="text-2xl font-semibold mb-4 text-primary-custom">{service.title}</h3>
                  <p className="text-neutral-custom leading-relaxed mb-6">{service.description}</p>
                  
                  <div className="mb-6">
                    <h4 className="font-semibold text-primary-custom mb-3">What's Included:</h4>
                    <ul className="space-y-2">
                      {service.features.map((feature, idx) => (
                        <li key={idx} className="flex items-center text-sm text-neutral-custom">
                          <div className="w-2 h-2 bg-accent-custom rounded-full mr-3"></div>
                          {feature}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="border-t pt-6">
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-bold text-accent-custom">{service.price}</span>
                      <Link href="/contact">
      ...
      <section className="py-20 gradient-bg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={additionalRef} className={`text-center mb-16 reveal ${additionalIntersected ? 'active' : ''}`}>
            <h2 className="text-4xl md:text-5xl font-bold mb-6 text-gradient">Additional Services</h2>
            <p className="text-xl text-neutral-custom max-w-3xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {additionalServices.map((service, index) => {
              ...
              return (
                <div
                  key={service.title}
                  className={`bg-white rounded-2xl p-8 shadow-lg border border-gray-100 reveal ${additionalIntersected ? 'active' : ''}`}
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                  <div className="w-12 h-12 bg-accent-custom/10 rounded-lg flex items-center justify-center mb-4">
                    <Icon className="w-6 h-6 text-accent-custom" />
                  </div>
                  <h3 className="text-xl font-semibold mb-3 text-primary-custom">{service.title}</h3>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={processRef} className={`text-center mb-16 reveal ${processIntersected ? 'active' : ''}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {process.map((step, index) => (
              <div
                key={step.step}
                className={`text-center reveal ${processIntersected ? 'active' : ''}`}
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="w-16 h-16 bg-accent-custom text-white rounded-full flex items-center justify-center mx-auto mb-6 text-2xl font-bold">
                  {step.step}
                </div>
                <h3 className="text-xl font-semibold mb-4 text-primary-custom">{step.title}</h3>
                <p className="text-neutral-custom leading-relaxed">{step.description}</p>
              </div>
            ))}
          </div>
        </div>
